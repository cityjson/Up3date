...
from .cityobjects import ExtensionObject, _AbstractCityObject, cityobject_from_dict
...
class Metadata:
    ...
    geographicalExtent: list[float] | None = None  # [minx, miny, minz, maxx, maxy, maxz]
    identifier: str | None = None
    pointOfContact: dict[str, Any] | None = None  # free-form per spec
    referenceDate: str | None = None             # ISO 8601 full-date, e.g. "1977-02-28"
    referenceSystem: str | None = None           # OGC CRS URI
    title: str | None = None
    extra: dict[str, Any] = field(default_factory=dict)  # non-standard / future fields

    @classmethod
    def from_dict(cls, data: dict) -> "Metadata":
        known = {"geographicalExtent", "identifier", "pointOfContact",
                 "referenceDate", "referenceSystem", "title"}
        return cls(
            ...
            referenceSystem=data.get("referenceSystem"),
    ...
    def to_dict(self) -> dict:
        ...
        if self.pointOfContact is not None:
            d["pointOfContact"] = self.pointOfContact
        if self.referenceDate is not None:
            d["referenceDate"] = self.referenceDate
        if self.referenceSystem is not None:
            d["referenceSystem"] = self.referenceSystem
...
class GeometryTemplates:
    ...
    vertices_templates: list[list[float]] = field(default_factory=list)  # JSON: "vertices-templates"
...
class CityJSONDocument:
    ...
    def from_dict(cls, data: dict) -> "CityJSONDocument":
        ...
        if "geometry-templates" in data:
            doc.geometry_templates = GeometryTemplates.from_dict(data["geometry-templates"])
        doc.CityObjects = {
            k: cityobject_from_dict(v) for k, v in data.get("CityObjects", {}).items()
        }
        return doc
    ...
    def to_dict(self) -> dict:
        d: dict[str, Any] = {
            ...
            "CityObjects": {k: v.to_dict() for k, v in self.CityObjects.items()},
